...
export async function onRequestPost(context) {
  ...
  const to = topicKey === 'grievance'
    ? (env.CONTACT_TO_GRIEVANCE || 'knoxdsahgo@proton.me')
    : (env.CONTACT_TO || 'contact@knoxvilledsa.org');
  ...
  //
  // `to` is always one of two fixed, trusted values chosen by topicKey, never
  // attacker input: the grievance topic routes to the HGO confidential
  // reporting channel, every other topic to CONTACT_TO. Do NOT add an
  // acknowledgement to `email`: that would derive a recipient from untrusted
  // input at the edge with no operator in the loop. See the header of
  // _house_theme.js.
